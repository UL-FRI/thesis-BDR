...
%\end{equation}
%
%Furthermore the weights used by the weighted sum action selection function (definition~\ref{def:animat:Sws:cwr}) were $w_\textnormal{s}=12$, $w_\textnormal{a}=8$ and $w_\textnormal{c}=8$. In my case, as already discussed, the modelled animal's per eye visual field was set to 150\deg and the visual range to seven body lengths. The other parameters were simply copied from Reynolds's model (\ie\ $m=1$, $ms=9$ and $\mf=27$). The weights were, on the other hand, set such that they gave the highest importance to repulsion from flockmates $w_\textnormal{r}=10$, followed by alignment with flockmates $w_\textnormal{p}=6$ and lowest to attraction to flockmates $w_\textnormal{a}=3$.
